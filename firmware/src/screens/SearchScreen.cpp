...
namespace VOXA
{
    ...
    ScreenId SearchScreen::show(Touch& touch)
    {
        ...
        float contentHeight = searchItems.size() * 50.0f + 10.0f;
        ...
        while (targetScreen == ScreenId::Search)
        {
            ...
            // 3. Render Search Screen
            ScreenCommon::renderSurface(canvas, w, h);
            ScreenCommon::renderHeader(canvas, "Recent Activities", true, false, Icon::Plus, w, h);

            // Render Back button highlight
            ...
            for (std::size_t i = 0; i < searchItems.size(); ++i)
            {
                ...
                canvas.drawString(searchItems[i].title.c_str(), leftX + 42.0f, cy - 8.0f);
