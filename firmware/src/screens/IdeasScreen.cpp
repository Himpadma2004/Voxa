...
namespace VOXA
{
    ...
    ScreenId IdeasScreen::show(Touch& touch)
    {
        ...
        float contentHeight = ideas.size() * 50.0f + 10.0f;
        ...
        while (targetScreen == ScreenId::Ideas)
        {
            ...
            if (touched && entryFrame >= 10)
            {
                ...
                else
                {
                    ...
                    if (!isSelectMode && m_pressedItemIndex != -1 && !m_isDragging && !longPressTriggered && (nowMs - pressStartMs > 500))
                    {
                        ...
                        selectedItems.clear();
                        selectedItems.resize(ideas.size(), false);
            ...
            else
            {
                if (m_wasTouched)
                {
                    ...
                    if (m_isDragging)
                    {
                        ...
                        {
                            Serial.println("[Ideas] Triggering manual pull-to-refresh sync...");
                    ...
                    else
                    {
                        if (isMovePopupActive)
                        {
                            ...
                            {
                                std::string targets[3] = {"reminders", "questions", "others"};
                                std::string targetCat = targets[moveTargetIndex];
                                for (size_t i = 0; i < selectedItems.size(); ++i)
                                {
                                    if (selectedItems[i] && i < ideas.size())
                                    {
                                        dataService.moveItem("ideas", targetCat, ideas[i].id);
                                    }
                                }
                                isSelectMode = false;
                        ...
                        else if (isSelectMode)
                        {
                            ...
                            else if (m_isPinPressed)
                            {
                                for (size_t i = 0; i < selectedItems.size(); ++i)
                                {
                                    if (selectedItems[i] && i < ideas.size())
                                    {
                                        dataService.togglePin("ideas", ideas[i].id);
                                    }
                                }
                                isSelectMode = false;
                                selectedItems.clear();
                        ...
                        else
                        {
                            ...
                            {
                                DetailScreen::setItem("ideas", ideas[m_pressedItemIndex].id, ScreenId::Ideas);
            ...
            ScreenCommon::renderHeader(canvas, isSelectMode ? "Select Items" : "Ideas", true, true, isSelectMode ? Icon::Plus : Icon::Plus, w, h);
            ...
            for (std::size_t i = 0; i < ideas.size(); ++i)
            {
                ...
                std::string drawTitle = ideas[i].title;
                ...
                canvas.drawString(ideas[i].timestamp.c_str(), leftX + textOffset, cy + 8.0f);
            ...
            if (isMovePopupActive)
            {
                ...
                // Target categories
                std::string targetLabels[3] = {"Reminders", "Questions", "Other notes"};
