...
namespace VOXA
{
    ...
    ScreenId IdeasScreen::show(Touch& touch)
    {
        ...
        while (targetScreen == ScreenId::Ideas)
        {
            ...
            // 3. Render Ideas Screen
            ScreenCommon::renderSurface(canvas, w, h);
            ScreenCommon::renderHeader(canvas, "Ideas", true, true, Icon::Plus, w, h);
            ...
            for (std::size_t i = 0; i < ideas.size(); ++i)
            {
                ...
                canvas.drawString(ideas[i].title.c_str(), leftX + 42.0f, cy - 8.0f);

                canvas.setTextColor(subColor);
                canvas.drawString(ideas[i].timestamp.c_str(), leftX + 42.0f, cy + 8.0f);
