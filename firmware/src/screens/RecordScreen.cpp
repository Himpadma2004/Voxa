...
namespace VOXA
{
    ...
    ScreenId RecordScreen::show(Touch& touch)
    {
        ...
        while (targetScreen == ScreenId::Record)
        {
            ...
            if (uiState == UIState::Uploading) btnColor = 0xFBE0; // Amber/yellow for upload
            else if (uiState == UIState::Result) btnColor = 0x2508; // Green for success
            else if (uiState == UIState::Error) btnColor = 0xD104; // Darker red for error
